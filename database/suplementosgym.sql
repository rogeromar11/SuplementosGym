-- =============================================================
-- SuplementosGym — Instalador único (base de datos completa)
-- Base: suplementosgym  ·  MariaDB 10.4 / MySQL 8  ·  InnoDB  ·  utf8mb4
-- -------------------------------------------------------------
-- Ejecuta este archivo UNA vez sobre una base vacía y crea todas
-- las tablas, catálogos y datos iniciales:
--
--   · IonAuth: groups, users, users_groups, login_attempts
--   · Catálogo: countries, products, product_images, payment_methods
...
--
-- Datos iniciales: países (CR/SV), grupos, permisos, catálogos base,
--   configuraciones y administradores de desarrollo.
--
-- Administradores de desarrollo (contraseña: password):
--   · Costa Rica   → admin@admin.com
--   · El Salvador  → admin@elsalvador.local
--   (cambiar en producción)
--
-- Uso:
--   mysql -u root < suplementosgym.sql
-- =============================================================

CREATE DATABASE IF NOT EXISTS `suplementosgym`
...
-- =============================================================
-- Contenido inicial de store_settings por país
-- (editable luego en el admin: Configuración → Tienda web)
-- =============================================================
INSERT INTO `store_settings` (`country_id`, `key`, `value`, `description`)
SELECT c.`id`, t.`key`, t.`value`, t.`description`
FROM `countries` c
JOIN (
    SELECT 'whatsapp_number' AS `key`, '' AS `value`, 'Número de WhatsApp (con código de país)' AS `description`
    UNION ALL SELECT 'contact_email', '', 'Correo de contacto'
    UNION ALL SELECT 'business_hours', '', 'Horario de atención'
    UNION ALL SELECT 'shipping_cost', '0', 'Costo de envío'
    UNION ALL SELECT 'free_shipping_from', '0', 'Envío gratis a partir de (0 = desactivado)'
    UNION ALL SELECT 'instagram_url', '', 'Enlace a Instagram'
    UNION ALL SELECT 'facebook_url', '', 'Enlace a Facebook'
    UNION ALL SELECT 'tiktok_url', '', 'Enlace a TikTok'
    UNION ALL SELECT 'availability_low_stock', '1', 'Mostrar aviso de pocas unidades'
    UNION ALL SELECT 'availability_min_stock', '5', 'Existencias mínimas para mostrar "Disponible"'
    UNION ALL SELECT 'availability_show_qty', '1', 'Mostrar el número de unidades en el aviso'
) t
WHERE NOT EXISTS (
